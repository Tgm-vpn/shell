...
# Используем `-a` для автоматической установки без интерактивных запросов.
# Это запустит установку в /opt/3x-ui и настроит systemd сервис.
/tmp/install_3xui.sh -a || { log_error "Ошибка при запуске установочного скрипта 3x-ui."; exit 1; }
