...
# Добавляем sqlite3 к списку устанавливаемых пакетов
...
yes n | /tmp/install_3xui.sh || { log_error "Ошибка при запуске установочного скрипт 3x-ui."; exit 1; }
...
# Важно: В новой версии 3x-ui столбца 'enable' может не быть в таблице 'users'.
# Исправляем запрос, чтобы вставлять только username и password.
...
# Измененный запрос: удален 'enable, 1'
