...
# sqlite3 не нужен для прямого редактирования, но может быть полезен для отладки.
# Однако, для данного скрипта он больше не требуется в обязательном порядке.
...
log_info "Устанавливаем имя пользователя 3x-ui на '$THREEXUI_USERNAME'..."
/usr/local/x-ui/x-ui setting -username "$THREEXUI_USERNAME" || { log_error "Ошибка при установке имени пользователя."; exit 1; }
log_success "Имя пользователя 3x-ui установлено на '$THREEXUI_USERNAME'. 🔑"

# Устанавливаем пароль
log_info "Устанавливаем пароль для пользователя '$THREEXUI_USERNAME'..."
/usr/local/x-ui/x-ui setting -password "$THREEXUI_ROOT_PASSWORD" || { log_error "Ошибка при установке пароля."; exit 1; }
log_success "Пароль для пользователя '$THREEXUI_USERNAME' успешно установлен. 🔒"
