...
# Важно: При использовании `yes n` порт будет выбран случайным образом установочным скриптом 3x-ui.
# Мы перенастроим его позже через sqlite3.
...
log_info "1/4: Обновляем список пакетов и устанавливаем curl, git и sqlite3..."
apt update -y || { log_error "Ошибка при apt update."; exit 1; }
apt install -y curl git sqlite3 || { log_error "Ошибка при установке curl, git или sqlite3."; exit 1; }
...
# Измененный запрос: удален 'enable', так как этого столбца может не быть в новой версии 3x-ui
sqlite3 "$THREEXUI_DB" "INSERT INTO users (username, password) VALUES ('root', '$MD5_HASH');" || { log_error "Ошибка при добавлении пользователя 'root'."; exit 1; }
log_success "Пользователь 3x-ui 'root' с новым паролем успешно добавлен. 🔑"

# Важно: Сбросим WebBasePath, который мог быть случайно сгенерирован
log_info "Сбрасываем WebBasePath на '/' для прямого доступа."
sqlite3 "$THREEXUI_DB" "UPDATE settings SET value = '/' WHERE key = 'WEB_BASE_PATH';" || { log_error "Ошибка при сбросе WEB_BASE_PATH."; exit 1; }
